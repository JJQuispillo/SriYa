...
public class TypedRequestValidatorsTests
{
    ...
    private static EmisorBaseDto EmisorBaseWithSecuencial(string? secuencial) =>
        new("1792268071001", "Test Corp", "Av. Quito 123", "001", "001", secuencial);
    ...
    // ---------- Secuencial: formato-si-presente (Change #3 R6, MODIFIED "Secuencial format validation") ----------
    // El validador es agnóstico al modo del tenant: valida SÓLO el formato cuando el secuencial está
    // presente (CLIENT o AUTO-con-conflicto). La obligatoriedad en CLIENT y la prohibición en AUTO las
    // resuelve el handler (que conoce el modo). Trazas: S "Client mode rejects bad format" / "Auto mode allows absence".

    [Fact]
    public void Factura_Secuencial9NumericDigits_Passes()
    {
        // T-SEC-023: presente y 9 dígitos numéricos → pasa.
        var validator = new CreateFacturaRequestValidator();
        var request = new CreateFacturaRequest(EmisorBaseWithSecuencial("000000123"), Comprador(), [Item()]);

        validator.TestValidate(request).ShouldNotHaveValidationErrorFor("Secuencial");
    }

    [Theory]
    [InlineData("12345678")]   // 8 dígitos (corto)
    [InlineData("1234567890")] // 10 dígitos (largo)
    [InlineData("00000000A")]  // no numérico
    public void Factura_SecuencialPresentButInvalidFormat_Fails(string secuencial)
    {
        // T-SEC-024: presente pero no-9-dígitos / no-numérico → falla con el mensaje de formato.
        var validator = new CreateFacturaRequestValidator();
        var request = new CreateFacturaRequest(EmisorBaseWithSecuencial(secuencial), Comprador(), [Item()]);

        var result = validator.TestValidate(request);
        result.ShouldHaveValidationErrorFor("Secuencial")
            .WithErrorMessage("El secuencial del emisor debe tener exactamente 9 dígitos numéricos.");
    }

    [Theory]
    [InlineData(null)]
    [InlineData("")]
    public void Factura_SecuencialAbsent_Passes(string? secuencial)
    {
        // T-SEC-024 (S "Auto mode allows absence"): ausente/null → el validador NO falla (formato-si-presente).
        var validator = new CreateFacturaRequestValidator();
        var request = new CreateFacturaRequest(EmisorBaseWithSecuencial(secuencial), Comprador(), [Item()]);

        var result = validator.TestValidate(request);
        result.ShouldNotHaveValidationErrorFor("Secuencial");
    }
