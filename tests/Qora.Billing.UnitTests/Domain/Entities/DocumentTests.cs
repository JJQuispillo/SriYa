...
public class DocumentTests
{
    ...
    [Fact]
    public void Anonymize_ShouldRedactBuyerPii_RetainFiscalFields_AndSoftDelete()
    {
        var document = Document.Create(
            ...
            {
                { "razonSocialComprador", "Cliente Real" },
                { "identificacionComprador", "0102030405001" },
                { "emailComprador", "cliente@example.com" },
                { "direccionComprador", "Av. Real 123" },
            });

        document.Anonymize();

        document.IsAnonymized.Should().BeTrue();
        document.DeletedAt.Should().NotBeNull("anonimizar marca también el soft-delete");

        // REDACTA la PII del comprador.
        document.BuyerInfo["razonSocialComprador"].Should().Be("[ANONIMIZADO]");
        document.BuyerInfo["emailComprador"].Should().Be("[ANONIMIZADO]");
        document.BuyerInfo["direccionComprador"].Should().Be("[ANONIMIZADO]");
        // RETIENE la identificación fiscal del comprador y la identidad de negocio.
        document.BuyerInfo["identificacionComprador"].Should().Be("0102030405001");
        document.Estab.Should().Be("001");
        document.Secuencial.Should().Be("000000001");
    }
    ...
    public void Anonymize_IsIdempotent()
    {
        ...
        document.DeletedAt.Should().Be(firstDeletedAt, "anonimizar de nuevo no cambia la marca de borrado");
        document.IsAnonymized.Should().BeTrue();
    }

    [Fact]
    public void SoftDelete_ShouldSetDeletedAt_WithoutAnonymizing()
    {
        var document = CreateTestDocument();
        ...
        document.IsAnonymized.Should().BeFalse("el soft-delete puro no redacta PII");
    }
