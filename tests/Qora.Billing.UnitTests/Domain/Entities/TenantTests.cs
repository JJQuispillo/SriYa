...
public class TenantTests
{
    ...
    [Fact]
    public void Anonymize_ShouldRedactPiiAndCredentials_ButRetainFiscalFields()
    {
        var tenant = Tenant.Create("1792268071001", "Test Corp", "TestCo", "contacto@example.com");
        tenant.ConfigureEmail(
            emailEnabled: true,
            emailProvider: Qora.Billing.Domain.Enums.EmailProvider.Custom,
        ...
        // RETIENE: RUC y razón social (datos fiscales del emisor).
        tenant.Ruc.Value.Should().Be("1792268071001");
        tenant.BusinessName.Should().Be("Test Corp");

        // REDACTA: PII + credenciales SMTP, y desactiva.
        tenant.TradeName.Should().BeNull();
        tenant.ContactEmail.Should().BeNull();
        tenant.EmailEnabled.Should().BeFalse();
        tenant.SmtpHost.Should().BeNull();
        tenant.SmtpUser.Should().BeNull();
        tenant.SmtpPassword.Should().BeNull();
        tenant.SenderEmail.Should().BeNull();
        tenant.IsActive.Should().BeFalse();
    }
}
