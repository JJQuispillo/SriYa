...
public static class DocumentEndpoints
{
    ...
    private static Task<Created<DocumentResponse>> CreateFactura(
        ...
        ProcessTypedAsync(request, validator, r => r.ToCreateDocumentRequest(), tenantContext, http, sender, ct);
    ...
    /// El header opcional <c>Idempotency-Key</c> habilita el replay (II-1): un reintento con la misma
    /// clave y el mismo cuerpo devuelve la respuesta original sin reemitir al SRI.
    /// </summary>
        ...
        CancellationToken ct)
    {
        ...
        var idempotencyKey = http.Request.Headers.TryGetValue("Idempotency-Key", out var headerValue)
            ? headerValue.FirstOrDefault()
            : null;
        var command = new ProcessDocumentCommand(tenantId, map(request), idempotencyKey);
